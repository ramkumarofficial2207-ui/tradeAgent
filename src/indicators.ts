...
export function identifySetupType(ind: StockIndicators): 'Pullback Continuation' | 'Volatility Contraction (VCP)' | 'Breakout Base' {
    const { ltp, ema20, high3m, rsi14 } = ind;

    // Breakout Base: Price near 3M high + RSI recovering
    if (ltp >= high3m * 0.97 && rsi14 >= 44) return 'Breakout Base';

    // VCP: Multiple contractions — price near EMA20, RSI 35-45
    if (Math.abs(ltp - ema20) / ema20 < 0.02 && rsi14 < 45) return 'Volatility Contraction (VCP)';
