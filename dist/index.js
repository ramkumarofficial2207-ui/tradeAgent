...
const dataService_2 = require("./dataService");
...
app.get('/api/screener/stock/:ticker', async (req, res) => {
    try {
        ...
        if (forceRefresh)
            (0, fundamentalService_1.clearStockCache)(ticker);
        const data = await (0, fundamentalService_1.fetchStockReport)(ticker);
        if (!data) {
...
app.post('/api/trades/add', async (req, res) => {
    try {
        ...
        await (0, tradeManager_1.addTrade)(setup);
...
app.get('/api/trades/history', async (req, res) => {
    try {
        const history = await (0, tradeManager_1.getTradeHistory)();
...
app.post('/api/backtest', async (req, res) => {
    try {
        const body = req.body;
        const allTickers = Object.keys(dataService_2.NSE_UNIVERSE);
        // Allow custom ticker subset or use full universe based on universeSize
        const universeSize = body.universeSize ?? 60;
        const tickers = body.tickers?.length > 0
            ? body.tickers
            : universeSize > 0
        ...
        const result = await (0, backtester_1.runBacktest)(config, (done, total, ticker) => {
