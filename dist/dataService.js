...
exports.MARKET_CAP_CR_MAP = Object.fromEntries(Object.keys(exports.NSE_UNIVERSE).map((ticker) => [ticker, 5000]));
...
async function fetchNiftyData() {
    try {
        ...
        let niftyChange = 0;
        let vixChange = 0;
        if (niftyRes.status === 'fulfilled' && niftyRes.value.length >= 2) {
            const c = niftyRes.value;
            const prev = c[c.length - 2].close;
            const curr = c[c.length - 1].close;
            niftyChange = ((curr - prev) / prev) * 100;
        }
        if (vixRes.status === 'fulfilled' && vixRes.value.length >= 2) {
            const v = vixRes.value;
            const prev = v[v.length - 2].close;
            const curr = v[v.length - 1].close;
            vixChange = ((curr - prev) / prev) * 100;
        }
        return { niftyChange, vixChange };
    }
    catch {
        return { niftyChange: 0, vixChange: 0 };
    }
}
