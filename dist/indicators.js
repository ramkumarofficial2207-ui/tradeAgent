...
function detectVCP(candles, currentIdx) {
    ...
    if (contractionCount >= 2)
        quality += 2; // Double contraction is elite
    if (tightness < 7)
        quality += 2;
    if (tightness < 4)
        quality += 1; // Elite tight base
    if (volDryUp)
        quality += 1;
    if (breakoutVol)
        quality += 1;
    if (pctFromPivot < 2)
        quality += 1; // At the pivot — prime entry
...
function isBreakout(candles, lookback = 15) {
...
function identifySetupType(ind) {
    ...
    if (ltp < ema50 * 0.85 && rsi14 < 30)
        return 'Deep Value Reversion 📉';
    if (breakout && ltp >= high3m * 0.97 && rsi14 >= 44)
        return 'Breakout Base';
    if (Math.abs(ltp - ema20) / ema20 < 0.02)
        return 'EMA20 Bounce';
