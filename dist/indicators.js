...
exports.detectVCP = detectVCP;
exports.isBreakout = isBreakout;
...
function computeIndicators(ticker, candles, niftyCandles) {
    ...
    try {
        ...
        const adxArr = technicalindicators_1.ADX.calculate(adxInput);
        if (adxArr.length > 0)
            adx14 = adxArr[adxArr.length - 1].adx;
    }
    catch {
        adx14 = 0;
    }
...
function computeConfidence(ind, rr) {
    ...
    const rsiScore = ind.rsi14 >= 60 && ind.rsi14 <= 68 ? 0.8 :
        ind.rsi14 >= 55 && ind.rsi14 < 60 ? 0.6 :
            ind.rsi14 > 68 && ind.rsi14 <= 72 ? 0.6 : 0.0;
    const distScore = ind.distFrom200 >= 5 && ind.distFrom200 <= 20 ? 0.7 :
        ind.distFrom200 > 20 && ind.distFrom200 <= 30 ? 0.5 :
            ind.distFrom200 > 0 && ind.distFrom200 < 5 ? 0.3 : 0.0;
    const athScore = ind.pctFrom52wHigh >= 3 && ind.pctFrom52wHigh <= 10 ? 0.5 :
    ...
    if (ind.isBullFlag) {
        ...
        if (ind.ltp <= ind.dma200)
            total = Math.min(total, 4.0); // Below 200DMA
        if (ind.rsi14 < 55 || ind.rsi14 > 80)
            total = Math.min(total, 5.0); // RSI out of zone
        if (ind.volumeRatio < 1.2)
            total = Math.min(total, 5.5); // Weak volume
    }
...
function computeRegime(niftyCandles, vixLevel) {
    const closes = niftyCandles.map(c => c.close);
    const dma50Arr = technicalindicators_1.SMA.calculate({ period: 50, values: closes });
    const dma200Arr = technicalindicators_1.SMA.calculate({ period: 200, values: closes });
...
function detectVCP(candles, currentIdx) {
    const EMPTY = { isVCP: false, quality: 0, tightness: 99, contractionCount: 0, pivotPrice: 0, pctFromPivot: 99 };
    ...
    if (peakIdx > lookback * 0.65 || (lookback - peakIdx) < 10)
        return EMPTY;
...
function isBreakout(candles, lookback = 20) {
...
function identifySetupType(ind) {
    ...
    if (ind.isDeepValue)
        return 'Deep Value Reversion 📉';
    const vcp = detectVCP(ind.candles);
    if (vcp.isVCP && vcp.quality >= 7)
        return 'VCP Breakout 🔥';
    if (vcp.isVCP)
    ...
    if (ltp >= high52w * 0.97 && rsi14 >= 55)
        return 'Momentum Continuation';
    if (breakout && ltp >= high3m * 0.97 && rsi14 >= 55)
        return 'Breakout Base';
    if (Math.abs(ltp - ema20) / ema20 < 0.02 && ema50 > (ind.dma200 ?? 0))
        return 'EMA20 Pullback';
    if (Math.abs(ltp - ema50) / ema50 < 0.025)
